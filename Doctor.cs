...
namespace StructureOfHospital
{
    ...
    {
        [EnumMember(Value = "kardiologist")]
        Kardiolog,

        [EnumMember(Value = "urologist")]
        Urolog,

        [EnumMember(Value = "neurologist")]
        Neurolog,

        [EnumMember(Value = "laryngologist")]
        Laryngolog
    }
    ...
    public class Doctor : Worker, ISerializable
    {
        ...
        public int DisplayList()
        {
            ...
            {
                Console.WriteLine("{0}: {1} {2} pesel: {3} specialty: {4} pwz: {5}", i + 1, DoctorList[i]._name,
        ...
        public int DisplayWholeList()
        {
            ...
            for (i = 0; i < DoctorList.Count(); i++)
            {
                Console.WriteLine("{0}: {1} {2} pesel: {3} specialty: {4} pwz: {5} login: {6} Password: {7}", i + 1, DoctorList[i]._name,
                                 DoctorList[i]._surname, DoctorList[i]._pesel, DoctorList[i]._specialty.ToString().ToLower(),
        ...
        public Doctor(SerializationInfo info, StreamingContext context)
        {
            ...
            _pzw = (int)info.GetValue("PWZ", typeof(int));
        }
    }
}